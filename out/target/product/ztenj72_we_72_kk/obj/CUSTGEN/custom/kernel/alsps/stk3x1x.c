...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	15
	#define STK_LT_N_CT	5
	#define STK_HT_N_CT	8
...
#endif
#ifdef STK_ALS_REVISE
//	#define STK_REVISE_LEN	10
	#define STK_REVISE_LEN	2
	#define MAX_REVISE_LEN 50
struct data_revise {
    u16 raw[MAX_REVISE_LEN];
    u16 max;
    u16 min;
    u32 avg;
    int idx;
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	else
	{
...
#endif
#ifdef STK_ALS_REVISE
		if(obj->revise.idx <= STK_REVISE_LEN)
		{
			if(als_data > obj->revise.max)
				obj->revise.max = als_data;
				
			if(als_data < obj->revise.min)
				obj->revise.min = als_data;
				
			if(als_data == 0)
				obj->revise.minidx++;
				
			obj->revise.raw[obj->revise.idx] = als_data;
			obj->revise.avg += als_data;
			
			if(obj->revise.idx != 0)
			{
				als_data = obj->als_last;
			}
			obj->revise.idx++;
		}
		else
		{
			obj->revise.avg = obj->revise.avg/STK_REVISE_LEN;
			
			if(obj->revise.minidx >= 3)
				als_data = 0;
			if(obj->revise.avg < 5)
				als_data = 0;
			else
				als_data = obj->revise.avg;
			
			obj->revise.max = 0;
			obj->revise.min = 0xFFFF;
			obj->revise.avg = 0;
			obj->revise.idx = 1;
			obj->revise.minidx = 0;
		}
		...
		{
			als_data = 0;
			obj->als_last = als_data;
		}
		else if(abs(als_data - obj->als_last) >= STK_ALS_CODE_CHANGE_THD)
		{
			obj->als_last = als_data;
		}
		else
...
#endif
#ifdef STK_ALS_REVISE
	memset(&obj->revise, 0x00, sizeof(obj->revise));  
#endif
#ifdef STK_TUNE0
...
//	APS_DBG("I2C = %d, addr =0x%x\n",stk31xx_force[0],stk31xx_force[1]);
	if(i2c_add_driver(&stk3x1x_i2c_driver))
	{
		APS_ERR("add driver error\n");
		return -1;
