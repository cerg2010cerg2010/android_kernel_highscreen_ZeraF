...
static int kxtf9_local_init(void);
static int kxtf9_remove(void);
static int kxtf9_init_flag =0; // 0<==>OK -1 <==> fail
...
static int KXTF9_SetDataResolution(struct kxtf9_i2c_data *obj)
{
	...
	if(hwmsen_read_byte_sr(obj->client, KXTF9_REG_DATA_RESOLUTION, databuf))//<==modified By MTK
	{
...
static int KXTF9_CheckDeviceID(struct i2c_client *client)
{
	...
	if(res <= 0)
	{
       ...
       if(databuf[0]!=KXTF9_FIXED_DEVID)
	{
...
*/
	...
	if (res <= 0)
	{
	       printk("\r\n zhaoshaopeng KXTF9_CheckDeviceID fail\r\n");
		return KXTF9_ERR_I2C;
	}
...
static int KXTF9_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(hwmsen_read_byte_sr(client, addr, databuf))//<==modified By MTK
	{
...
static int KXTF9_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if(hwmsen_read_byte_sr(client, KXTF9_REG_DATA_FORMAT, databuf))//<==modified By MTK
	{
		printk("kxtf9 read Dataformat failt \n");
		return KXTF9_ERR_I2C;
	}
...
static int KXTF9_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	if(hwmsen_read_byte_sr(client, KXTF9_REG_BW_RATE, databuf))//<==modified By MTK
	{
...
static int kxtf9_init_client(struct i2c_client *client, int reset_cali)
{
	struct kxtf9_i2c_data *obj = i2c_get_clientdata(client);
	int res = 0;
       printk("\r\n zhaoshaopeng kxtf9_init_client start\r\n");
	...
	printk("\r\n zhaoshaopeng kxtf9_init_client success \r\n");
#ifdef CONFIG_KXTF9_LOWPASS
...
static ssize_t show_register_value(struct device_driver *ddri, char *buf)
{
		...
		if(hwmsen_read_byte_sr(obj->client, i2c_dev_reg, databuf))//<==modified By MTK
		{
			GSE_ERR("read power ctl register err!\n");
			return KXTF9_ERR_I2C;
		}
...
/*----------------------------------------------------------------------------*/
static int kxtf9_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
static int kxtf9_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
...
static int kxtf9_resume(struct i2c_client *client)
{
	...
	if(err = KXTF9_SetPowerMode(obj->client,true))
	{
...
//		return;        
//	}
	if(err = KXTF9_SetPowerMode(obj->client,true))//<==Added by MTK
	{
	GSE_ERR("initialize client fail!!\n");
	return; 
	}//<==Added by MTK
...
static int kxtf9_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("\r\n zhaoshaopeng kxtf9_i2c_probe \r\n");
	memset(obj, 0, sizeof(struct kxtf9_i2c_data));

	obj->hw = kxtf9_get_cust_acc_hw();
	...
	atomic_set(&obj->trace, ADX_TRC_RAWDATA);
	atomic_set(&obj->suspend, 0);
	...
	if((err = kxtf9_create_attr(&(kxtf9_init_info.platform_diver_addr->driver))))
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = kxtf9_gsensor_operate;
...
static int kxtf9_i2c_remove(struct i2c_client *client)
{
	...
	if(err = kxtf9_delete_attr(&(kxtf9_init_info.platform_diver_addr->driver)))
	{
...
{
	struct acc_hw *hw = kxtf9_get_cust_acc_hw();
	GSE_FUN();
...
static int __init kxtf9_init(void)
{
	...
	hwmsen_gsensor_add(&kxtf9_init_info);
